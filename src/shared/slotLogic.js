 ...
 */
export function isSlotBusy(slotStart, slotEnd, busyPeriods) {
 ...
 */
export function generateDaySlots(date, durationMin, busyPeriods, businessHours = DEFAULT_BUSINESS_HOURS) {
    const slots = [];
 ...
 * 複数日にわたるスロットを生成する。
 */
export function generateTimeSlotsFromBusyPeriods(from, to, durationMin, busyPeriods, businessHours = DEFAULT_BUSINESS_HOURS) {
    const slots = [];
